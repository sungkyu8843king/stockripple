/**
 * 실시간 이벤트 수집 + 즉시 AI 분석 트리거
 * - 트럼프 Truth Social 신규 글
 * - 경제지표 발표 결과 (actual 값 등장 시)
 * - 기업 실적 발표 결과 (epsActual 등장 시)
 */
...
export default async function handler(req, res) {
  ...
  try {
    const r = await fetch(`${base}/api/trump`, { signal: AbortSignal.timeout(12000) });
    const j = await r.json();
    for (const post of j.items || []) {
      if (!post.text || !post.publishedAt) continue;
      // 6시간 이내 글만 수집 (오래된 것 재수집 방지)
  ...
  // ─── 2. 경제지표 발표 결과 수집 ─────────────────────────────────────────
  try {
    const r = await fetch(`${base}/api/economic`, { signal: AbortSignal.timeout(12000) });
    const j = await r.json();

    for (const item of j.items || []) {
      // actual 값이 있어야 "발표된" 지표
