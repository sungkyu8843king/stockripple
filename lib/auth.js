...
export async function verifyAdmin(authHeader) {
  ...
  // 스케줄 Claude Code 에이전트 전용 — 기존 ADMIN_SECRET/CRON_SECRET(GitHub Actions 등
  // 다른 자동화가 공유)과 분리된 별도 시크릿이라 재발급해도 다른 파이프라인에 영향 없음.
  if (process.env.AGENT_SECRET && token === process.env.AGENT_SECRET) return { ok: true, mode: 'agent' };
