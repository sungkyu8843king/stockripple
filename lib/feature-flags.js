...
export const FEATURE_FLAG_DEFS = [
  ...
  { key: 'rank_reason',          label: '실시간 랭킹 사유 (매수판단 없는 순수 서술)' },
  { key: 'deep_dive',            label: '심층분석 (여러 기사 종합 장문 게시물)' },
  { key: 'chat',                 label: '실시간 채팅 위젯 (OFF면 사이트에 아예 안 뜸)' },
  { key: 'expose_ripple_effects', label: '매수후보/신뢰도/파급효과 공개 노출 (유사투자자문업 리스크, OFF 기본값 — analyze 파이프라인 on/off와 무관)' },
];
...
// fail-closed 변형 — 행이 없거나 조회 실패 시 기본 비활성화. 법적/보안 리스크가 있는
// 스위치(chat, expose_ripple_effects)는 안전장치 부재 시 꺼진 채로 남아야 하므로 이걸 쓴다.
export async function isFeatureEnabledStrict(supabase, key) {
  try {
    const { data } = await supabase.from('feature_flags').select('enabled').eq('key', key).maybeSingle();
