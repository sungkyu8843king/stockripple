...
- 종목명이 티커 코드 그대로 등록되는 버그가 반복 발생 — 어드민 액션 `fix-kr-broken-names`로 일괄 정리(`adf52c2`). **2026-08-05 확장**: 이 핸들러가 원래 `.eq('market','KR')`이라 KR만 고쳤는데, 실측 결과 이름이 티커뿐인 156건 중 **107건이 US/기타 시장**이었다(`name_ko='AXP'` 등) — 한국어로 "아메리칸 익스프레스"를 검색해도 안 나오고 검색 드롭다운에 코드가 그대로 노출됐다. 지금은 시장 구분 없이 전부 대상이고, 이름 소스는 **네이버 자동완성(`ac.stock.naver.com/ac`) 하나로 통일**했다 — `api/stock.js`의 `handleSearchKr`이 이미 쓰던 엔드포인트인데 거기선 KOSPI/KOSDAQ만 남기고 버렸었다. 실측상 미국 종목의 한글명도 준다(적중률 87%, 실패분은 THK·CATL·ABB 같은 비미국 상장이라 **덮어쓰지 않고 스킵**). 어드민 패널 "전략투자 관리" 탭의 **🏷 종목명 일괄 보정** 버튼이 done까지 자동으로 이어 호출한다.
  - ⚠️ 네이버 **해외주식 상세 API(`api.stock.naver.com/stock/<code>.<거래소>/basic`)는 쓰지 말 것** — 거래소 접미사를 미리 알아야 하는데 `.O`(나스닥)만 되고 `.N`(NYSE)은 409를 반환한다(JPM·AXP·C 실측). 접미사가 필요 없는 위 자동완성 경로를 쓰는 이유다.
  - ⚠️ 이 핸들러의 `start`/`nextStart`는 **"소비한 건수"가 아니라 "누적 실패 건수"**다. 고쳐진 종목은 다음 호출의 대상 목록에서 아예 빠져 목록이 줄어들기 때문에, 소비 개수만큼 건너뛰면 미처리 종목을 통째로 넘겨버린다(다음 호출 시점 목록은 항상 `[이번에 실패한 것들] + [아직 안 본 것들]` 순). 다른 resumable 핸들러(`crawl-etf-holdings` 등)는 대상 목록이 안 줄어서 소비 개수 방식이 맞으니 그대로 복사해 오지 말 것.
  - ⚠️ `companies`는 이미 1459행이라 **PostgREST 1000행 상한에 걸린다** — 시장 필터 없이 한 번에 select하면 459행이 조용히 누락된다. 이 핸들러는 `range()`로 페이지네이션한다. `companies` 전체를 훑는 코드를 새로 짤 때 같은 함정에 빠지지 말 것.
...
### `chat.js` — `@` 멘션(종목/모의투자 첨부, 2026-08)

`chat_messages.message`는 순수 텍스트 컬럼(1~300자 CHECK, 구조화 컬럼 없음)이라, 첨부는 스키마 변경 없이 메시지 본문 안에 컴팩트 마커를 심는 방식으로 구현했다 — `[[stock:TICKER:이름]]`(종목), `[[port:총자산:수익률%:종목수]]`(모의투자 스냅샷). `renderChatBody()`가 렌더 시점에 이 마커를 카드로 치환한다.

- **종목 마커는 이름까지 마커 안에 박아둔다**(`[[stock:005930.KS:삼성전자]]`) — 렌더할 때 회사명 조회를 또 안 하려는 것. 가격만 렌더 후 배치로 채운다(`schedulePriceFill()`, 200ms 디바운스) — 초기 로드 50건이 전부 종목 멘션이면 `/api/quotes`를 50번 부르는 걸 막기 위함.
- **모의투자 마커는 "지금 스냅샷"이지 실시간 링크가 아니다** — `paper_portfolios`/`paper_positions`의 RLS(`auth.uid() = user_id`)는 다른 사용자가 남의 포트폴리오를 읽는 걸 막으므로, 채팅을 보는 다른 사용자 입장에선 라이브 조회 자체가 불가능하다. 그래서 "@모투"를 선택하는 그 순간 보내는 사람의 브라우저가 자기 계좌를 즉시 계산해(총자산·수익률·보유종목수) 숫자를 메시지 텍스트에 그대로 찍어 넣는다 — 이후 시세가 바뀌어도 이 메시지의 숫자는 안 바뀐다(분석 이슈 공유 이미지 카드와 같은 "스냅샷" 원칙).
- **자동완성은 새 검색 로직을 만들지 않고 `hFindMatches()`(site-header.js 전역, 헤더검색·랭킹탭검색과 동일 함수)를 그대로 재사용**한다. 결과 행 스타일도 `site-header.js`가 이미 전역 주입한 `.hsug-item`류 클래스를 그대로 쓴다(새 CSS 최소화) — 다만 그 드롭다운(`.hsug`)은 위쪽 검색창 기준 "아래로" 펼쳐지는 포지션이라, 패널 맨 아래 입력창에는 안 맞아서 위로 펼쳐지는 `.src-mention` 래퍼만 새로 뒀다.
- 마커에 들어가는 이름은 자동완성으로만 채워지므로(사용자가 직접 대괄호/콜론을 타이핑해 넣는 경로 없음) `sanitizeMentionPart()`로 구분자(`[`,`]`,`:`) 문자만 방어적으로 제거한다.
