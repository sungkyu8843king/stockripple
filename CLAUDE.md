...
### ⓪ 해외 실시간 추정가 — 모델 튜닝 지점 (2026-07-31 추가)

국장 마감 중 "해외에서 거래되는 한국물"로 재개장 예상가를 역산하는 모듈. **정확도가 본질적으로 낮은 추정이라, 근거를 항상 화면에 드러내고 파라미터를 한 곳에 모으는 것이 설계 원칙**이다.
...
- **튜닝은 `kr-market.html`의 `EST_MODEL` 객체 하나만 고치면 된다** — `SIGNALS`(소스 해석기), `targets[].sources[]`의 `w`(신뢰 가중치, 1:1 대응일수록 크게)와 `beta`(그 종목의 소스 민감도). 서버는 raw signal만 주고 모델 파라미터를 갖지 않는다(재배포 없이 프론트만 고치려는 의도).
- 소스가 죽으면 자동으로 제외되고 **남은 소스로 가중치가 재정규화**된다. 카드에 반영 소스·신뢰도(소스 수 + 소스 간 표준편차 기반 4단계)·직전 종가를 노출해 추정 근거를 검증할 수 있다.
- 서버 원재료: `GET /api/market-data?source=kr-proxy` (`api/market-data.js`의 `handleKrProxy`) — EWY(MSCI 한국 ETF)·`^SOX`·`KRW=X`·ADR 7종(SKM/KB/PKX/LPL/WF/SHG/KEP). ADR 종목을 늘리려면 `KR_PROXY_SYMBOLS`에 `{kr:'…KS'}` 매핑을 추가.
- **⚠️ 바이낸스 주식 perp는 Vercel 서버에서 HTTP 451(지역 차단)로 막힌다**(2026-07-31 실측). 그래서 서버가 아니라 **브라우저에서 직접** 시도하는 선택적 소스(`tryBinanceClient`)로 분리해뒀다 — 사용자 네트워크에서도 막히면 '미연결'로 표시될 뿐 나머지로 정상 계산된다. 서버에서 다시 시도하게 바꾸지 말 것.
- **KOSPI 선물은 Yahoo에 없다** — `KSU=F`는 404, `^KS200`은 현물이라 장중에만 갱신된다. 선물 신호가 필요하면 다른 소스를 찾아야 한다.
- 화면에 **"이건 예상 수치입니다" 경고를 상단 고정**으로 두고 있다. 이 문구는 사용자가 명시적으로 요구한 것이니 임의로 빼지 말 것(유사투자자문업 리스크와도 직결).
